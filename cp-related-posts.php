...
if ( ! function_exists( 'cprp_gutenberg_editor' ) ) {
	function cprp_gutenberg_editor() {
		wp_enqueue_script( 'cprp_gutenberg_script', plugin_dir_url( __FILE__ ) . 'scripts/cprp_gutenberg.js', array(), '1.1.0' );
	}
}
...
function cprp_related_post_form( $post ) {
	...
	<div><input type="button" value="Get recommended tags" onclick="cprp_get_tags(<?php print esc_attr( $post->ID ); ?>);" class="button-primary" /></div>
	<div style="width:100%; height:150px; overflow: auto; border: 1px solid #CCC; margin-top: 10px;" id="cprp_tags">
	<?php
...
function cprp_load_admin_resources() {
	...
	if ( isset( $post ) && in_array( $post->post_type, $post_types ) ) {
		...
		wp_enqueue_style( 'cprp_admin_style', $plugin_dir_url . 'styles/cprp_admin.css', array(), '1.1.0' );
		wp_enqueue_script( 'jquery' );
		wp_enqueue_script( 'jquery-ui-sortable' );
		wp_enqueue_script( 'cprp_admin_script', $plugin_dir_url . 'scripts/cprp_admin.js', array( 'jquery', 'jquery-ui-sortable' ), '1.1.0', true );
...
function cprp_enqueue_scripts() {
	...
	wp_enqueue_script( 'cprp_script', $plugin_dir_url . 'scripts/cprp.js', $dependencies, '1.1.0', true );
	wp_enqueue_style( 'cprp_style', $plugin_dir_url . 'styles/cprp.css', array(), '1.1.0' );
	wp_localize_script(
		'cprp_script',
		'cprp',
		array(
	...
	function cprp_embed_google_ads() {
		if (
			...
			if ( ! empty( $ad_client ) && ! empty( $ad_unit ) ) {
				...
				wp_enqueue_script( 'google-adsense', '//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=' . esc_attr( $ad_client ), array(), '1.1.0' );
